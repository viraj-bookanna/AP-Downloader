import asyncio,logging,os,random,string,re,subprocess,time,shutil,platform,requests,dotenv,html,hashlib,urllib.parse
...
def save_playlist(json_data):
    if not os.path.isdir('playlists'):
        os.makedirs('playlists')
    filename = f"playlists/{json_data['title']}.m3u8"
    with open(filename, 'w') as f:
        f.write(json_data['playlist'])
    return f'http://127.0.0.1:{HTTP_SERVER_PORT}/'+urllib.parse.quote(filename)
...
```
1. Copy above code
2. Goto the video playing page in browser and type "j" in the url bar
...
async def handler(event):
    if event.message.document or 'json' in ("" if not event.message.file else event.message.file.mime_type):
        if event.message.file.size < 1024*10:
            return
        jfile = await event.message.download_media()
    ...
    os.makedirs(tmpdir)
    try:
        inFileName = save_playlist(jdata)
        orgFileName = f"{jdata['title']}.mp4"
        outFileName = hashlib.md5(orgFileName.encode()).hexdigest()+'.mp4'
        outFilePath = os.path.join(tmpdir, outFileName)
        cmd = ['python' if is_win else 'python3', 'converter.py', json.dumps({'i':inFileName,'o':outFileName,'h':jdata['headers']}).encode().hex() ]
        ...
        for i in range(parts):
            ...
            await upload_and_send(event,
                ...
                jdata['title'] if parts==1 else f"{jdata['title']} part {i}"
            )
        await msg.delete()
    except Exception as e:
...
exec_cmd(['python' if is_win else 'python3', '-m', 'http.server', str(HTTP_SERVER_PORT)])
with bot:
